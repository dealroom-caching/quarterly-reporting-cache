...
const SHEET_CONFIGS = [
  ...
  {
    key: "quarterly_funding",
    name: "Quarterly Funding Data",
    gid: "883461842",
  },
  { key: "yearly_ev", name: "Yearly Enterprise Value", gid: "1785838256" },
  {
    key: "top_industries_tags",
    name: "Top Industries and Tags",
    gid: "1075248480",
  },
  { key: "top_rounds", name: "Top Rounds", gid: "279039385" },
  { key: "config", name: "Config", gid: "491582669" },
];
...
async function fetchSheetData(
  ...
  const filteredRows = allRows.filter((row) => {
    // For the 'config' sheet, we don't use the PRIMARY_KEY filter
    if (config.key === "config") return true;
  ...
  console.log(
    `   [${config.name}] Raw: ${allRows.length} -> Filtered: ${filteredRows.length} rows`,
  );
...
async function main() {
  ...
  try {
    ...
    // Get reporting config from the 'config' sheet
    const configRows = sheetsData["config"] || [];
    const configMap: Record<string, string> = {};
    configRows.forEach((row) => {
      // Support both 'key'/'value' and 'reporting_quarter' etc as direct columns
      ...
      if (key) {
        ...
        // Fallback: if the sheet has columns like 'reporting_quarter' directly
        Object.entries(row).forEach(([k, v]) => {
          if (!configMap[k]) configMap[k] = v;
        });
      }
    });

    const reportingQuarter =
      configMap["reporting_quarter"] ||
      `${now.getFullYear()}Q${Math.ceil((now.getMonth() + 1) / 3)}`;
    const reportingYear = parseInt(
      configMap["reporting_year"] || now.getFullYear().toString(),
    );
    const reportingQuarterNumber = parseInt(
      configMap["reporting_quarter_number"] ||
        Math.ceil((now.getMonth() + 1) / 3).toString(),
    ) as 1 | 2 | 3 | 4;
    ...
    const output = {
      meta: {
        ...
        reporting_quarter_number: reportingQuarterNumber,
        schema_version: "2.0",
      },
      sheets: sheetsData,
      config: {
